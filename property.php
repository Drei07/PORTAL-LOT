...
<div id="page-wrapper">
					...
					<div class="col-lg-8">
                        <div class="row">
							...
							<?php 
                              ...
                              {
                                $query=mysqli_query($con,"SELECT property.*, user.uname,user.utype,user.uimage FROM `property`,`user` WHERE property.uid=user.uid and property.status = 'available' ");
                              }
                              else{
                                $query=mysqli_query($con,"SELECT property.*, user.uname,user.utype,user.uimage FROM `property`,`user` WHERE property.uid=user.uid and property.status = 'available' and property.uid != {$_SESSION['uid']}");
                              }
 ...
 if(isset($_SESSION['uid']) == '')
 {
    ...
    $query=mysqli_query($con,"SELECT * FROM `property` WHERE isFeatured = 1 and status = 'available' and uid != {$_SESSION['uid']} ORDER BY date DESC LIMIT 3");
 }
...
if(isset($_SESSION['uid']) == '')
{
    $query=mysqli_query($con,"SELECT * FROM `property` where status = 'available'  ORDER BY date DESC LIMIT 6");
}
else{
    $query=mysqli_query($con,"SELECT * FROM `property` where status = 'available' and uid != {$_SESSION['uid']} ORDER BY date DESC LIMIT 6");
}
